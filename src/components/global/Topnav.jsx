...
const Topnav = () => {
    ...
    return (
        <section className='h-20 flex justify-between bg-white pb-1.5'>
            ...
            <div className={`${hide} absolute right-0 left-0 top-0 bottom-0 bg-white z-10 py-5 px-4 md:hidden`}>
                <div className='flex justify-end items-center' onClick={toggleMenu}>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </div>
                <Link className='my-3 block mr-4 font-semibold text-gray-600 hover:text-lime-500' to='#'>F.A.Q.</Link>
                <Link to='/' className='my-3 block outline-none rounded text-sm py-1 px-4 bg-gradient-to-b from-red-500 to-red-900 text-white hover:opacity-90'>Log In</Link>
                <Link to='/signup' className='my-3 block outline-none rounded text-sm py-1 px-4 bg-gradient-to-b from-red-500 to-red-900 text-white hover:opacity-90'>Sign Up</Link>
            </div>
        </section>
    );
}
